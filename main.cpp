...
// alignas(64) prevents false sharing by forcing each Event packet to fit // perfectly into a standard CPU cache line size. struct alignas(64) Event { uint64_t id; uint64_t timestamp_ns; char payload[32]; };

// ============================================================================ // LAYER 2: PRE-ALLOCATED LOCK-FREE ENGINE // ============================================================================

template<typename T, size_t Capacity> class LockFreeConcurrentBroker { static_assert((Capacity & (Capacity - 1)) == 0, "Capacity must be a power of 2 for bitwise masking.");

private: struct Cell { std::atomic<size_t> sequence; T data; };
...
public: LockFreeConcurrentBroker() { for (size_t i = 0; i != Capacity; ++i) { buffer_[i].sequence.store(i, std::memory_order_relaxed); } }
...
constexpr size_t OPERATIONS_PER_PRODUCER = 250000; constexpr size_t NUM_PRODUCERS = 4; constexpr size_t TOTAL_EVENTS = OPERATIONS_PER_PRODUCER * NUM_PRODUCERS; constexpr size_t BROKER_CAPACITY = 1024; // Must be power of 2

void producer_thread(size_t id, LockFreeConcurrentBroker<Event, BROKER_CAPACITY>& broker) { for (size_t i = 0; i < OPERATIONS_PER_PRODUCER; ++i) { Event ev; ev.id = (id * OPERATIONS_PER_PRODUCER) + i; std::string payload_str = "TELEMETRY_PACKET_" + std::to_string(i % 100); std::strncpy(ev.payload, payload_str.c_str(), sizeof(ev.payload));
...
void consumer_thread(LockFreeConcurrentBroker<Event, BROKER_CAPACITY>& broker, std::atomic<size_t>& consumed_count, std::atomic<uint64_t>& total_latency_ns) { size_t local_consumed = 0; uint64_t local_latency = 0;
